...
class Network:
    ...
    def SGD_train(self, train, epochs, eta, lam=0.01, verbose=True, test=None):
        """
    ...
    def back_prop(self, x, y):
        ...
        delta_L = self.gradC(a_list[-1], y) * self.g_prime(z_list[-1])
        db_list[(self.L-2)] = delta_L
        dW_list[(self.L-2)] = np.dot(delta_L, a_list[-2].T)
        for l in range(self.L-2,0,-1):
            interim = np.dot(self.weights[l].T, delta_L)
            z = z_list[l]
            sp = self.g_prime(z)
            delta_L = interim * sp
            db_list[l-1] = delta_L
            dW_list[l-1] = np.dot(delta_L, a_list[l-1].T)
...
if __name__ == "__main__":
    ...
    # change path to ../data/tinyMNIST.pkl.gz after debugging
    ...
    #nn = Network([2,30,2])
    #hidden layer1 = cuberoot(n/m)*m*2
    #hidden layer2 = cuberoot(n/m)*m
    nn = Network([196,72,26,10])
